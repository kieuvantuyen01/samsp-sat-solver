...
\begin{table}[htbp]
\centering
\caption{Đặc trưng 4 test types trong bài báo gốc. ``Storage acts'': số activities
  có storage event; ``LB'': cận dưới CPM cho một copy.}
\label{tab:testtypes}
...
\paragraph{Hệ quả cho generator: 5 điều chỉnh so với phiên bản trước.}

\begin{enumerate}
  \item \textbf{Thêm tham số \texttt{multi\_event}:} cho phép một activity có cả $c_{is} > 0$
    lẫn $c_{is'} < 0$ (chuyển cuvette từ storage $s'$ sang $s$ trong một bước).
    Đây là cấu trúc phổ biến trong test types thực (ví dụ: ``Cold → Hot-II'').

  \item \textbf{Bất đối xứng capacity:} mặc định $P_s$ không đồng nhất;
    ít nhất một storage nên có capacity nhỏ hơn (bottleneck).
    Đề xuất: \texttt{storage\_caps = [4, 4, 4, 2]} làm default.

  \item \textbf{Lag distribution tách biệt:} tham số \texttt{lag\_range} riêng
    cho chemical process lags, không phụ thuộc vào \texttt{d\_range}.
    Ví dụ: \texttt{d\_range = [3, 7]}, \texttt{lag\_range = [10, 30]}.

  \item \textbf{Generator phải tái tạo được Fig.~4:} lịch 2 copies của test type Fig.~3
    với makespan 17 là ground truth dễ kiểm chứng. Nên thêm vào test suite.

  \item \textbf{Naming convention:} dùng \texttt{I[<type,copies>,...]} giống bài báo
    để dễ đối chiếu. Ví dụ: \texttt{SAMSP-SYN-1x5-seed42} thay vì tên tùy tiện.
\end{enumerate}

\paragraph{Benchmark sets đề xuất (cập nhật).}

\begin{table}[htbp]
\centering
\caption{Benchmark sets cập nhật sau phân tích bài báo gốc.}
\label{tab:benchmarks-updated}
\begin{tabularx}{\textwidth}{@{}p{0.24\textwidth}ZYY@{}}
\toprule
\textbf{Set} & \textbf{N} & \textbf{Phạm vi activities} & \textbf{Nguồn}\\
\midrule
RCPSP-Regression & 480+480 & 32--62 & PSPLIB $j30$/$j60$.\\
SAMSP-Original Set~I & 24 & 27--242 & Paper gốc (liên hệ tác giả).\\
SAMSP-Original Set~II & 24 & 97--1442 & Paper gốc (liên hệ tác giả).\\
SAMSP-Synthetic-Easy & 100 & 10--50 & Generator, $j=1$--5, 1 type.\\
SAMSP-Synthetic-Medium & 100 & 50--250 & Generator, $j=5$--20, 1--2 types.\\
SAMSP-Synthetic-Hard & 50 & 100--400 & Generator, $j=20$--60, 2--4 types.\\
SAMSP-UNSAT & 20 & any & Generator (positive cycle + storage vi phạm).\\
\bottomrule
\end{tabularx}
\end{table}

Với SAMSP-Synthetic, công bố: generator source, seeds, cấu hình và SHA-256 checksum.
